...
class _HomePageState extends State<HomePage> {
    ...
    return FutureBuilder(
      ...
      builder: (BuildContext _context, AsyncSnapshot _snapshot) {
        if (_snapshot.hasData) {
          ...
          num _selectedPrice = _exchangeRates[selectedCurrency.toLowerCase()];
          ...
          return Column(
            ...
            children: [
              GestureDetector(
                onDoubleTap: ()async {
                  String? result = await Navigator.push(context, MaterialPageRoute(builder: (BuildContext _context){
                    return DetailsPage(rates: _exchangeRates, onCurrencySelected: (String currency){
                      _selectedCurrency = currency.toLowerCase();
                    },);
                  },),);
                  if(result != null){
                    setState(() {
                      _selectedCurrency = result.toLowerCase();
                    });
                  }
                },
              ...
              _currentPriceWidget(_selectedPrice, selectedCurrency.toUpperCase()),
  ...
  Widget _currentPriceWidget(num _rate, String _currency) {
    return Text(
      '${_rate.toStringAsFixed(2)} $_currency',
      style: const TextStyle(
        color: Colors.white,
        fontSize: 30,
